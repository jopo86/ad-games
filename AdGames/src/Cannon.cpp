...
void Cannon::Run()
{
    ...
    Renderable cannonBall = Renderable::ColoredCircle(BALL_RADIUS, BALL_SEGMENTS, Vec4::Black());

    std::list<Quadruple<Vec2, Vec2, float, float>> cannonBalls;
    ...
    while (window.isOpen())
    {
        ...
        if (input.isMouseButtonPressed(MouseButton::Left))
        {
			...
			cannonBalls.push_back({ pos, dir * 500.0f, Math::Rand<float>(0.0f, 360.0f), Math::Rand<float>(50.0f, 360.0f) });
        }
        ...
        std::list<Quadruple<Vec2, Vec2, float, float>>::iterator it = cannonBalls.begin();
        while (it != cannonBalls.end())
        {
            ...
            ball.third += ball.fourth * dt;
            cannonBall.setPosition(Vec3(ball.first.getX(), ball.first.getY(), -1.0f));
            cannonBall.setRotation(Vec3(0.0f, 0.0f, ball.third));
